...
> 본 문서는 `1-domain-definition.md`(도메인 정의서 v1.6), `2-PRD.md`(PRD v1.2), `3-user-scenario.md`를 기반으로, 5일/1인 개발 규모의 MVP에 맞는 실용적 구조 원칙만 정의한다. 이 규모를 넘어서는 레이어링·추상화·조직 규칙은 다루지 않는다.
...
- **API 엔드포인트**: REST 자원 기준 `/api/<복수형-자원>` + 필요시 하위 액션.
  ...
  - `POST /api/events/:id/close` (회식 종료 처리 — 팀장 전용, C4의 선행 조건)
