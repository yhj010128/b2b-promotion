...
- **작업**
  ...
  - 상태값(`모집중/확정/종료`) 표시
- **완료 조건**
  - [x] 팀장 계정에서 일정 등록/저장이 성공함
  - [x] 팀원 계정에서 저장 버튼이 렌더링되지 않고 읽기 전용으로 표시됨
  - [x] 상태값이 도메인 용어 그대로 화면에 표시됨
  - [x] 768px 이하에서 폼이 단일 컬럼으로 정상 표시됨
...
- **완료 조건**
  - [x] 팀원/팀장 모두 의견을 제출할 수 있음
  - [x] 제출 성공 시 완료 메시지가 표시됨
  - [x] 미인증 상태에서 접근 시 로그인 화면으로 이동함(C1)
  - [x] 768px 이하에서 폼이 정상 표시됨
...
- **완료 조건**
  - [x] 추천 후보가 카드 형태로 식당명·예상 비용·만족도와 함께 표시됨
  - [x] **C2**: 팀원 화면에 확정 버튼이 렌더링되지 않음
  - [x] **C3**: 예산 미입력 응답 시 지정 문구가 화면에 표시됨
  - [x] **C6**: 후보 0건 응답 시 지정 문구와 수동 확정 입력이 팀장에게만 표시됨
  - [x] 확정 성공 시 화면이 확정 상태로 갱신됨
  - [x] 768px 이하에서 카드가 세로 1열로 쌓임
...
- **완료 조건**
  - [x] 상태가 `종료`인 회식에서 별점/한줄평 제출이 성공함
  - [x] **C4**: 상태가 `종료`가 아니면 제출 버튼이 비활성화되고 `"회식 종료 후 평가 가능"`이 표시됨
  - [x] 별점 미선택 시 제출이 차단됨
  - [x] 768px 이하에서 정상 표시됨
