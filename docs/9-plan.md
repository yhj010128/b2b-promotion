...
- **완료 조건**
  ...
  - [ ] 팀원이 일정 등록 API 호출 시 403이 반환됨
  - [ ] 팀장이 종료 처리를 하면 상태가 `종료`로 전이됨(F2 선행 조건 충족)
...
- **완료 조건**
  - [ ] 로그인한 사용자가 의견을 제출하면 `preferences`에 `event_id`, `user_id`와 함께 저장됨
  - [ ] 미인증 요청은 401로 거부됨(C1)
  - [ ] 동일 사용자 재제출 동작이 정해진 정책대로 일관되게 처리됨
...
- **완료 조건**
  - [ ] **C3**: 예산 NULL 상태 추천 요청 시 도메인 정의서 문구 그대로 오류가 반환되고 추천이 실행되지 않음
  - [ ] **C5**: 만족도 2.5점 미만 식당이 후보에서 제외되지 않고 최하위 순번으로 반환됨
  - [ ] **C5**: 최근 3회 이내 방문 식당도 최하위 순번으로 반환됨
  - [ ] **C6**: 예산 조건 충족 식당이 없을 때 지정 문구가 반환됨
  - [ ] 추천 호출 후에도 DB에 추천 결과가 저장되지 않음(비영속 확인)
  - [ ] C3/C5/C6 단위 테스트가 통과함
...
- **완료 조건**
  - [ ] 팀장이 확정하면 `confirmed_restaurant_id`가 저장되고 상태가 `확정`으로 전이됨
  - [ ] **C2**: 팀원이 확정 API 호출 시 403이 반환되고 데이터가 변경되지 않음
  - [ ] **C6**: 추천 후보 0건 상황에서 팀장의 수동 확정이 성공함
  - [ ] 확정 후 식당의 최근 방문 정보가 갱신되어 다음 추천의 C5 판단에 반영됨
...
- **완료 조건**
  - [ ] 상태가 `종료`인 회식에 대해 평가가 저장됨
  - [ ] **C4**: 상태가 `모집중`/`확정`일 때 평가 입력이 거부되고 지정 문구가 반환됨
  - [ ] 저장된 `reviews.restaurant_id`가 해당 회식의 확정식당과 일치함
  - [ ] 평가 저장 후 식당 누적 평균 점수가 재계산되어 갱신됨
  - [ ] 갱신된 점수가 다음 추천(B-5)의 C5 판정에 반영됨
  - [ ] 누적 평균 계산 단위 테스트가 통과함
...
- **완료 조건**
  - [ ] 로그인 성공 시 토큰이 저장되고 보호 화면으로 이동함
  - [ ] 로그인 실패 시 오류 메시지가 표시됨
  - [ ] **C1**: 미인증 상태로 보호 화면 접근 시 `ProtectedRoute`가 로그인 화면으로 리다이렉트함
  - [ ] access token 만료 시 자동 재발급 후 원 요청이 재시도됨
  - [ ] 768px 이하에서 레이아웃이 세로로 정렬되어 깨지지 않음
