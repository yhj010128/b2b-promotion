...
| 버전 | 날짜       | 변경 내용                           |
| ---- | ---------- | ----------------------------------- |
| 1.4  | 2026-08-20 | 실제 백엔드 구현과 정합성 재검토: 6장의 `/auth/refresh` 오타를 실제 경로 `/api/auth/refresh`로 정정 |
| 1.3  | 2026-08-13 | docs 정합성 재검토: 회식 "확정→종료" 전이 주체를 팀장으로 명시(F2 수용 기준), 참조 도메인 정의서 버전 v1.6으로 갱신 |
...
> 본 문서는 `docs/1-domain-definition.md` (TeamBab 도메인 정의서 v1.6)를 기반으로 작성되었다. 엔티티/용어/제약사항(C1~C6)/추적표는 도메인 정의서를 그대로 재사용하며, 본 문서는 이를 "5일/1인 개발" 제약 하에서 실행 가능한 제품 요구사항으로 구체화한다.
...
- 회식 상태 "확정"→"종료" 전이는 **팀장의 종료 처리**로만 발생한다(자동 전이 없음). 이 처리가 C4의 선행 조건이다.
...
- **인증**: JWT access token(예: 15분 만료) + refresh token(예: 14일 만료) 방식.
  ...
  - refresh token: `사용자` 테이블에 해시로 저장, `/api/auth/refresh` 엔드포인트에서 access token 재발급에만 사용. access token 만료 시 프론트에서 자동 재발급 요청 후 재시도.
  - 로그아웃/재발급 시 refresh token 회전(rotation), 탈취 대비 별도 블랙리스트·다중 세션 관리는 이번 범위 제외(향후 과제).
- 테이블은 도메인 정의서 5장의 엔티티 중 사용자, 회식 일정, 선호 의견, 만족도 평가, 식당을 스키마화하고, `사용자` 테이블에 `refresh_token_hash` 컬럼을 추가한다. **추천 결과는 도메인 정의서 5장(v1.6)에 명시된 대로 비영속** — 매 요청 시 재계산해 응답으로만 반환하고 별도 테이블을 두지 않는다 (필요 시 감사 로그 목적 저장은 향후 검토).
