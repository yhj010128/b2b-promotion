...
> 본 문서는 `docs/1-domain-definition.md` (TeamBab 도메인 정의서 v1.4)를 기반으로 작성되었다. 엔티티/용어/제약사항(C1~C6)/추적표는 도메인 정의서를 그대로 재사용하며, 본 문서는 이를 "5일/1인 개발" 제약 하에서 실행 가능한 제품 요구사항으로 구체화한다.
...
- 테이블은 도메인 정의서 5장의 엔티티 중 사용자, 회식 일정, 선호 의견, 만족도 평가, 식당을 스키마화하고, `사용자` 테이블에 `refresh_token_hash` 컬럼을 추가한다. **추천 결과는 도메인 정의서 5장(v1.4)에 명시된 대로 비영속** — 매 요청 시 재계산해 응답으로만 반환하고 별도 테이블을 두지 않는다 (필요 시 감사 로그 목적 저장은 향후 검토).
